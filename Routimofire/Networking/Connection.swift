...
class ConnectionUser {
    ...
    class func createUserWith(params params: [String : AnyObject]) {
        Alamofire.request(RouterUser.CreateUser(params)).responseJSON(completionHandler: {
            response in
    ...
    class func testUserWithoutParams() {
        Alamofire.request(RouterUser.TestUser).responseJSON(completionHandler: {
            response in
            switch response.result {
            case .Success(let JSON):
                print(JSON)
    ...
    class func updateUserWith(name name: String, params: [String : AnyObject]) {
        Alamofire.request(RouterUser.UpdateUser(name, params)).responseJSON(completionHandler: {
            response in
            switch response.result {
            case .Success(let JSON):
                print(JSON)
